...
function UpdateProduct() {
    ...
    const product= allProductsData.find((p)=>String(p._id)===productId);
    console.log(product);
  ...
  const [isWearable, setIsWearable]=useState(product?.isWearable || false);
  const [sizes, setSizes]=useState<string[]>(product?.sizes || []);
  const [replacementDays, setReplacementDays]=useState(product?.replacementDays|| "");
  const [warranty,setWarranty]=useState(product?.warranty ||"");
  const [freeDelivery, setFreeDelivery]=useState(product?.freeDelivery||false);
  const [payOnDelivery, setPayOnDelivery]=useState(product?.payOnDelivery||false);
  const [image1,setImage1]=useState<File|null>(null);
  const [image2,setImage2]=useState<File|null>(null);
  const [image3,setImage3]=useState<File|null>(null);
  const [image4,setImage4]=useState<File|null>(null);
  const [preview1,setPreview1]=useState<string|null>(product?.image1 || null);
  const [preview2,setPreview2]=useState<string|null>(product?.image2 ||null);
  const [preview3,setPreview3]=useState<string|null>(product?.image3 ||null);
  const [preview4,setPreview4]=useState<string|null>(product?.image4|| null);
  const [detailPoints,setDetailPoints]=useState<string[]>(product?.detailsPoints || []);
  const [currPoint,setCurrPoint]=useState<string>("");
  const [pointIndex, setPointIndex]=useState(detailPoints.length||0);
  ...
  return (
    <div className="px-4 pt-20 pb-10 bg-gradient-to-br from-gray-600 via-black to-gray-600 text-white min-h-screen">
      ...
      >
        ...
        disabled={loading}
         className="w-full mt-8 bg-blue-600 hover:bg-blue-700 py-3 rounded-lg font-semibold">{loading ?<ClipLoader size={22}/>: "Update Product"}</motion.button>

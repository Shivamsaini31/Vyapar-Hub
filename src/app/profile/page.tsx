...
function Profile() {
    ...
    const [previewImage, setPreviewImage]=useState(user?.image || defaultProfile);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-600 via-black to-gray-600 text-white px-4 pt-24 pb-10">
      ...
      className="max-w-3xl mx-auto bg-white/10 backdrop-blur-md p-6 sm:p-10 rounded-2xl border border-white/20 shadow-xl">
        <div className="flex flex-col items-center text-center">
            ...
            <p className="text-gray-400 text-xs sm:text-sm mt-1">Role: <span className="capitalize text-blue-400">{user?.role}</span></p>
        </div>
        ...
        <div className='grid grid-cols-1 gap-4 mt-8'>
            {user?.role==="user" && (
                ...
                onClick={()=>router.push("/orders")} className="bg-green-600 hover:bg-green-700 py-3 rounded-lg font-semibold">
                    My Orders
                </motion.button>
            )}
            <motion.button
                whileHover={{scale:1.02}}
                onClick={()=>{setShowEditProfile(!showEditProfile); setShowEditShop(false)}} className="bg-blue-600 hover:bg-blue-700 py-3 rounded-lg font-semibold">
                    Edit Profile
                </motion.button>
                {user?.role==="vendor" && (
                <motion.button
                whileHover={{scale:1.02}}
                onClick={()=>{setShowEditShop(!showEditShop);setShowEditProfile(false)}} className="bg-gray-600 hover:bg-gray-700 py-3 rounded-lg font-semibold">
                    Edit Shop details
                </motion.button>
            )}
        ...
        className='mt-10 bg-white/5 p-5 sm:p-6 rounded-xl border border-white/20'>
            ...
            <div className="flex flex-col items-center mb-6">
                ...
                hover:border-blue-400 mb-3 cursor-pointer'>
                    ...
                    height={120} width={120} className="object-cover w-full h-full"/>
                </motion.div>
                <label className="cursor-pointer bg-blue-600 px-4 py-2 rounded-lg text-sm">Select Image
                    <input type="file" accept="image/*" className="hidden" onChange={handlePreviewImage}/>
                </label>
            </div>
